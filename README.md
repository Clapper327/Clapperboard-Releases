...
[Clapperboard 3.25 Windows 릴리스](https://github.com/Clapper327/Clapperboard-Releases/releases/tag/v3.25)

설치 파일: `Clapperboard_Setup_3.25.exe`

설치 프로그램은 현재 사용자 영역의 `%LOCALAPPDATA%\Programs\Clapperboard`에 앱을 설치하고 시작 메뉴 바로가기를 만듭니다. 같은 설치 파일을 다시 실행하면 기존 설치본 위에 업데이트됩니다.

## 3.25 주요 변경사항

- English 언어의 Ko-fi 후원 창과 QR, 여백을 약 20% 작게 다듬었습니다.
- 한글 카카오페이 후원 창은 기존 크기를 그대로 유지합니다.
- v3.24 앱의 `지금 업데이트`에서 다운로드, 크기/SHA256 검증, 무인 설치와 v3.25 재실행까지 실제 원클릭 흐름을 검증합니다.
- Sync, 슬레이트 감지, 파형 미리보기와 무음 제거 동작은 변경하지 않았습니다.
- Windows 번들에는 FFmpeg와 ffprobe `8.1.2`가 계속 포함됩니다.
...
- 파일: `Clapperboard_Setup_3.25.exe`
- 크기: `81,178,502` bytes
- SHA256: `B44C0D6D3EFC1AD5D6F09DCB442B0FDAE2259326FE9AD9F4C75CD64888A02629`

PowerShell에서 확인:

```powershell
Get-FileHash .\Clapperboard_Setup_3.25.exe -Algorithm SHA256
```
...
macOS v3.25 pkg는 서명, 공증, 권한 처리와 실제 구버전 원클릭 업데이트 검증을 마친 뒤 별도로 배포합니다. 그전까지 공개 업데이트 피드의 `mac` 값은 `null`로 유지됩니다.
